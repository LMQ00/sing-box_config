...
where curl >nul 2>&1
if %errorlevel% neq 0 (
    echo [X] 错误：未找到 curl 命令，请先安装 curl 或升级到 Windows 10 1803+。
    pause
    exit /b 1
)
...
if not defined TAG (
    echo [X] 错误：无法获取版本号，可能是网络问题或 API 限流。
...
if not defined FOUND_BINARY (
    echo [X] 错误：在压缩包中未找到 sing-box.exe
    rmdir /s /q "%TEMP_DIR%" 2>nul
    exit /b 1
)
...
echo [OK] sing-box (%PLATFORM%) 下载并安装成功！
exit /b 0
...
echo [WARN] 警告：配置文件中检测到未替换的 '%PLACEHOLDER%'！
echo    程序可能无法正常运行。
set "confirm="
...
tasklist /fi "imagename eq sing-box.exe" 2>nul | findstr /i "sing-box.exe" >nul 2>nul
if errorlevel 1 (
    echo [X] 错误：Sing-box 启动失败，请查看 run\sing-box.log。
    pause
    goto menu
)
...
if not defined DASHBOARD_URL (
    echo [i]  config.json 中未配置 external_ui_download_url，跳过 Dashboard 下载。
    exit /b 0
)
...
if not exist "%TEMP_DIR%" mkdir "%TEMP_DIR%"
if errorlevel 1 (
    echo [WARN]  无法创建临时目录 %TEMP_DIR%，跳过 Dashboard 下载。
    exit /b 0
)
...
if "%DL_OK%"=="0" (
    echo [WARN]  Dashboard 所有下载链接均失败，跳过。
    rmdir /s /q "%TEMP_DIR%" 2>nul
    exit /b 0
)
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "try{$e='%TEMP_DIR%\extracted';Expand-Archive -Path '%ZIP_FILE%' -DestinationPath $e -Force;$i=Get-ChildItem $e;if($i.Count-eq1-and$i[0].PSIsContainer){$s=$i[0].FullName;Get-ChildItem $s|Copy-Item -Destination '%DASHBOARD_DIR%' -Recurse -Force}else{Get-ChildItem $e|Copy-Item -Destination '%DASHBOARD_DIR%' -Recurse -Force};Write-Host '[OK] Dashboard 部署完成！路径: %DASHBOARD_DIR%'}catch{exit 1}"

if %errorlevel% neq 0 (
    echo [WARN]  Dashboard 解压失败，跳过。
)
...
:update
echo [*] 更新订阅链接
echo [i] 提示：如果只输入一个链接，它将被复制到所有三个位置。
...
powershell -NoProfile -ExecutionPolicy Bypass -NoLogo -Command ^
    ...
    "if(-not $c.Contains($ph)){Write-Host '[WARN] 配置文件中未找到占位符（可能已配置过订阅），未做修改。'; exit 0};" ^
    "$u1=Read-Host '请输入 订阅1 链接';" ^
    "if([string]::IsNullOrWhiteSpace($u1)){Write-Host '[X] 错误：你没有输入任何链接！'; exit 1};" ^
    ...
    "if($left-gt 0){Write-Host ('[WARN] 仍有 {0} 个占位符未替换（订阅数量少于 3 时正常）。' -f $left)};" ^
    "Write-Host '';" ^
    "Write-Host '[OK] 成功！配置文件已更新。';" ^
    "foreach($i in 0..2){" ^
    "  $v=@($u1,$u2,$u3)[$i];" ^
...
if %errorlevel% neq 0 (
    echo [X] 更新失败（未输入链接或 PowerShell 异常）！
    pause
    goto menu
)
...
if exist "cache.db" (
    ...
    echo    [OK] 已删除 cache.db
) else (
    echo    [i]  cache.db 不存在，跳过
)
...
tasklist /fi "imagename eq sing-box.exe" 2>nul | findstr /i "sing-box.exe" >nul 2>nul
if not errorlevel 1 (
    echo [WARN]  sing-box 正在运行，请先停止（结束 sing-box.exe 进程）再更新内核。
    pause
    goto menu
)
