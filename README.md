...
**Windows 下 start.bat 报「'xxx' 不是内部或外部命令」/ 命令行错乱**

`start.bat` 被以 UTF-8 保存了。cmd.exe 在 UTF-8（65001）代码页下解析含中文的批处理文件会出现读取偏移错乱，命令被截断执行（例如把注释行中的 `>` 当成重定向、生成多余文件）。请保持 `start.bat` 为 **GBK（cp936）编码 + CRLF 换行**（脚本内已是 `chcp 936`），不要在脚本中使用 emoji。
